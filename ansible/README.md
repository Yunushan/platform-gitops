...
`playbooks/prepare-nodes.yml` opens the required RKE2 and Cilium overlay ports in firewalld when firewalld is active. `make rke2-network-check` verifies that joining nodes can reach the first server on the RKE2 supervisor/API ports after the first server is listening.
