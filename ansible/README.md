...
`playbooks/prepare-nodes.yml` opens the required RKE2 and Cilium overlay ports in firewalld, trusts the RKE2 pod CIDR and Cilium interfaces when firewalld is active, and keeps NetworkManager away from Kubernetes CNI interfaces. `make rke2-network-check` verifies that joining nodes can reach the first server on the RKE2 supervisor/API ports after the first server is listening.
