...
help:
	...
	@echo "  forge-workspace-audit-users  Read-only audit of Forgejo user presence and account flags"
...
forge-workspace-import:
	...
	@$(PYTHON) scripts/forge_workspace.py import "$(PLAN)" --snapshot "$(SNAPSHOT)" --work-dir "$(WORK_DIR)" $(if $(PROOF),--proof "$(PROOF)",) $(if $(filter 1 true yes,$(CONFIRM_MAIL_DELIVERY)),--confirm-mail-delivery,) $(if $(filter 1 true yes,$(NO_SEND_NOTIFY)),--no-send-notify,) $(if $(PASSWORD_FILE),--password-file "$(PASSWORD_FILE)",)

forge-workspace-audit-users:
	@test -n "$(PLAN)" || (echo "PLAN=private/migrations/gitlab-to-forgejo.workspace.json is required" >&2; exit 2)
	@test -n "$(SNAPSHOT)" || (echo "SNAPSHOT=private/migrations/proof/workspace-snapshot.json is required" >&2; exit 2)
	@$(PYTHON) scripts/forge_workspace.py audit-users "$(PLAN)" --snapshot "$(SNAPSHOT)" $(if $(PROOF),--proof "$(PROOF)",)
