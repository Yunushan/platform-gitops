...
- First-party API clients have finite socket deadlines and bounded response
  bodies. Oversized or invalid transport configuration must fail before remote
  JSON is parsed or retained in proof artifacts.
...
| Security and supply chain | SOPS or external secrets, image/chart pinning, exact upstream chart package provenance with declared local patches, CI action SHA, execution-image digest, exact runtime patch pinning, hash-locked amd64/arm64 CI wheels, checksum-pinned CI release binaries without runtime module resolution, no-network digest-pinned Semgrep, parser fuzzing, branch-coverage evidence, update review, release-time Cosign proof, and admission-time signature verification are in place |
...
| Local file input is bounded | `python scripts/test_bounded_file_contract.py`; production evidence, plans, inventories, configurations, rendered manifests, and binary hash inputs pass through the shared 64 MiB reader, invalid or over-512 MiB overrides fail closed before parsing, and non-regular inputs are opened nonblocking then rejected |
| JSON parsing is unambiguous and bounded | `python scripts/test_strict_json_contract.py`; all first-party production decoders reject duplicate object keys, non-standard numeric constants, finite-range overflow, structures deeper than 128 containers, and structures above 1,000,000 nodes while classifying decoder recursion as invalid input |
| YAML parsing is unambiguous and bounded | `python scripts/test_strict_yaml_contract.py`; rendered evidence rejects duplicate keys, anchors and aliases, non-JSON tags and mapping keys, non-finite numbers, streams above 10,000 documents, structures deeper than 128 containers, and structures above 1,000,000 nodes while classifying decoder recursion and malformed input without echoing document content |
| API transport is bounded and credential-safe | `python scripts/test_http_transport_contract.py`; direct HTTP calls use the shared fail-closed opener, require HTTPS for credential headers, reject URL credentials and every 3xx before forwarding authorization, bound URLs/queries/headers/request and response bodies plus CLI fallback payloads, and reject unsafe timeout or byte-limit overrides |
...
| Runtime capacity verified | `make platform-capacity-verify`; every node and Longhorn retain the configured filesystem, scheduler, and storage headroom, platform storage is on a filesystem distinct from `/`, encrypted StorageClasses and CSI Secret references are valid, and every bound Longhorn volume is encrypted |
...
`PLATFORM_PRODUCTION_SCORE_ENV_FILE` to select a dedicated configuration;
`PLATFORM_PRODUCTION_EVIDENCE_PYTHON` and
`PLATFORM_PRODUCTION_SCORE_PYTHON` may select dedicated interpreters. Explicitly
