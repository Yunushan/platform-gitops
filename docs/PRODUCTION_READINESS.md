...
| API transport is bounded and redirect-safe | `python scripts/test_http_transport_contract.py`; direct HTTP calls use the shared fail-closed opener, reject every 3xx before forwarding authorization headers, declare finite timeouts, bound response/error bodies and CLI fallback payloads, and reject unsafe timeout or byte-limit overrides |
