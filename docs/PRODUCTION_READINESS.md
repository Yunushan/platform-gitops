...
| Security and supply chain | SOPS or external secrets, image/chart pinning, exact upstream chart package provenance with declared local patches, CI action SHA and execution-image digest pinning, parser fuzzing, branch-coverage evidence, update review, release-time Cosign proof, and admission-time signature verification are in place |
