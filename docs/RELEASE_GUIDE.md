...
compilation and behavior tests, before building the release bundle. Only that
verified bundle crosses into the privileged publication job, which publishes:
...
release policy. This keeps write and OIDC permissions behind an approval after
the read-only verification job succeeds.
